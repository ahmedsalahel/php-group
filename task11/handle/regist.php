...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($username)) {
        if (strlen($username) > 3) {
            if (preg_match('/Eng/', $username)) {
                $flag++;
            } else {
                $all_errors['preg_un'] = 'plz fill username contain Eng';
            }
        } else {
    ...
    if (!empty($password)) {
        if (strlen($password) > 8) {
            if (preg_match('@[A-Za-z]@', $password)) {
                ...
                $all_errors['preg_pw'] = 'plz fill password contain Eng';
            }
        } else {
    ...
    if ($myimg['error'] != 4) {
        if ($img_size < 2097152) {
            if (in_array($img_ext, $allowed_ext)) {
                if ($flag == 2) {
                    ...
                    $stat = "insert into new_users (name , password , image) values ('$username' , '$password' , '$img_name')";
        ...
        if ($flag == 2) {
            $stat = "insert into new_users
            (name , password ) values ('$username' , '$password')";
            $query = mysqli_query($connection, $stat);
            header('location:../login.php');
        }
    }
